...
// Todos os roles suportados pelo backend
        ...
        <Card className="border-glow bg-card overflow-hidden">
          ...
          <CardContent className="p-6 -mt-10 relative">
            <div className="flex items-end gap-4">
              ...
              <Badge variant="outline" className="ml-auto mb-1 text-xs gradient-text border-primary/30">
          ...
          <Card className="border-glow bg-card h-full">
            <CardContent className="p-6">
              ...
              <div className="space-y-3">
                ...
                <div><Label className="text-xs text-muted-foreground">Função</Label>
                  <p className="text-sm font-medium">{ROLES.find(r => r.value === user?.role)?.label ?? user?.role}</p>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                {users.map(u => {
                  ...
                  return (
                    <div key={u.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/30 transition-colors group">
                      ...
                      {/* ADMIN pode alterar role de outros */}
                      {user?.role === 'ADMIN' && !isMe ? (
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <SelectTrigger className="h-7 w-32 text-[10px] bg-muted border-border/50">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              {ROLES.map(r => (
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent className="border-glow bg-card">
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => remove.mutate(u.id)} className="bg-destructive text-white">Remover</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </div>
