...
import { useAuth } from '@/contexts/AuthContext'; // Importação necessária para Onboarding
...
export default function PedidosPage() {
  ...
  const { orders, isLoading } = useOrders(statusFilter === 'TODOS' ? undefined : statusFilter as OrderStatus);
  const [searchTerm,    setSearchTerm]    = useState('');
  const [isSheetOpen,   setIsSheetOpen]   = useState(false);

  const filtered = (orders ?? []).filter(order => {
    const nomeCliente = (order.clienteNome || "").toLowerCase();
    const idPedido    = (order.id || "").toLowerCase();
    const termo       = searchTerm.toLowerCase();

    const matchSearch = nomeCliente.includes(termo) || idPedido.includes(termo);
  ...
  return (
      <div className="space-y-6">
        {/* Regra 5: Passo a Passo Obrigatório */}
        ...
        >
          <Button
              ...
              disabled={!user?.onboardingCompleted} // Bloqueia ações se não concluiu guia
          >
              ...
              {isLoading ? (
                  ...
                  filtered.map(order => (
                      <TableRow key={order.id} className="border-border/20 hover:bg-primary/5 transition-colors">
                        ...
                        <TableCell>
                          {/* Usa os campos achatados do DTO atualizado */}
